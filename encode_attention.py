...
def main(args):
    ...
    encoder = SequenceEncoderAttn(models, task.target_dictionary)
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        extract_matrix = []
        i = 0
        for outputs in encodings:
            if i >= args.n_points:
                break
            # Raw attention matrix per head
            a = outputs[1]['attn']
            # Vector-norm attention matrix per head
            afx = outputs[1]['norms'][1]
            # Vector-norm attention matrix per head, summed heads
