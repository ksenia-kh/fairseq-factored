...
PATH = "/home/usuaris/veu/ksenia.kharitonova/tfm/data/europarl/en-fr/en-fr-joined-bpe"
LANG = 'en'
...
def main():
    for dataset in ['corpus.tc.10000', 'dev', 'test']:
        ...
        with open(os.path.join(PATH, dataset + '.' + LANG + '_tags'), 'r') as file:
            text_tag = file.read()
        '''
        with open(os.path.join(PATH, dataset + '.' + LANG + '_lemmasS'), 'r') as file:
            text_lemma = file.read()
        with open(os.path.join(PATH, dataset + '.' + LANG + '_posS'), 'r') as file:
            text_pos = file.read()
        with open(os.path.join(PATH, dataset + '.' + LANG + '_depsS'), 'r') as file:
            text_dep = file.read()
        with open(os.path.join(PATH, dataset + '.' + LANG + '_tagsS'), 'r') as file:
            text_tag = file.read()
