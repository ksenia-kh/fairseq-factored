...
FTDIR="/home/usuaris/veu/ksenia.kharitonova/tfm/data/sanders-gender-debias/handcraft_no"
...
subword-nmt apply-bpe -c ${DEST_DIR}/${TRN_PREF}.codes.${SRC}-${TGT} --vocabulary ${DEST_DIR}/${TRN_PREF}.vocab.${SRC} --vocabulary-threshold 50 < ${FTDIR}/saunders_hd_no.en > ${FTDIR}/saunders_hd_no.bpe.${SRC}
subword-nmt apply-bpe -c ${DEST_DIR}/${TRN_PREF}.codes.${SRC}-${TGT} --vocabulary ${DEST_DIR}/${TRN_PREF}.vocab.${TGT} --vocabulary-threshold 50 < ${FTDIR}/saunders_hd_no.es > ${FTDIR}/saunders_hd_no.bpe.${TGT}
...
stdbuf -i0 -e0 -o0  $PYTHON $FAIRSEQ_DIR/preprocess.py --source-lang $SRC --target-lang $TGT \
    --trainpref $FTDIR/saunders_hd_no.bpe --destdir $FTDIR  \
