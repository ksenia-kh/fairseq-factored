...
stdbuf -i0 -e0 -o0 $PYTHON $FAIRSEQ_DIR/train.py $WORKING_DIR \
 ...
 --warmup-updates 0 --lr 0.0001 --min-lr 1e-09 --dropout 0.1 --weight-decay 0.0001 \
 --criterion label_smoothed_cross_entropy --label-smoothing 0.1 --max-tokens 4000 \
  --restore-file $CP_DIR/checkpoint_last.pt --reset-dataloader --reset-lr-scheduler --reset-meters --save-dir $CP_DIR \
  --source-lang en_tokensS --target-lang es_tokensS --max-update 4000
